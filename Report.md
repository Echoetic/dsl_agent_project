...
sequenceDiagram
    ...
    participant G as GeminiRecognizer
    participant L as LocalRecognizer
...
```mermaid
graph TD
    S[Script] --> E[entry: welcome]
    S --> V[variables: name, amount]
    S --> ST1[Step: welcome]
    S --> ST2[Step: registration]
    S --> ST3[Step: goodbye]
    
    ST1 --> SP1[SpeakStatement]
    SP1 --> EX1[Expression: 字符串拼接]
    EX1 --> STR1["您好，"]
    EX1 --> VAR1[$name]
    EX1 --> STR2["，请问有什么可以帮您？"]
    
    ST1 --> LS1[ListenStatement]
    LS1 --> T1[start: 5]
    LS1 --> T2[end: 30]
    
    ST1 --> BR1[Branch: 挂号 -> registration]
    ST1 --> BR2[Branch: 查询 -> query]
    ST1 --> SIL[Silence: silenceHandler]
    ST1 --> DEF[Default: defaultHandler]
```
...
stateDiagram-v2
    [*] --> IDLE: 创建会话
    IDLE --> RUNNING: start()
    RUNNING --> RUNNING: 执行Speak
    RUNNING --> WAITING_INPUT: 执行Listen
    WAITING_INPUT --> RUNNING: process_input()
    RUNNING --> FINISHED: 执行Exit
    RUNNING --> ERROR: 运行时异常
    WAITING_INPUT --> FINISHED: 超时退出
    FINISHED --> [*]
    ERROR --> [*]
```
